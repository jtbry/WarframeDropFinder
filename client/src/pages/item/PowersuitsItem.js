    ...
    <div>
      ...
      <Card style={{display: 'flex', marginTop: '-1rem',  position: 'relative', zIndex: '1'}}>
        <CardContent style={{lineHeight: '1rem'}}>
          <h1 style={sectionNameStyles}>{item.name} {item.wikiaUrl && <Button style={{marginLeft: '3rem'}} href={`${item.wikiaUrl}`} target="_blank">Wiki Page <ArrowRightAltIcon/> </Button>}</h1>
          ...
          <br />
          <ItemPricing item={item} seperator={<ArrowRightOutlinedIcon/>} />
        </CardContent>
      </Card>
      {/* Render Components */}
      {item.components && <h2 style={componentSectionNameStyles}>Components</h2>}
